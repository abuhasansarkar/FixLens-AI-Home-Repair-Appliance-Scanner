...
export const attachmentForNormalization = internalQueryGeneric({ args: { attachmentId: v.id("assistantAttachments") }, returns: v.any(), handler: async(ctx,args)=>{const user=await requireUser(ctx);return requireOwned(await ctx.db.get(args.attachmentId),user._id);} });

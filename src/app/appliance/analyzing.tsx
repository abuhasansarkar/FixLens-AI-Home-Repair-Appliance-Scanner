...
export default function ApplianceAnalyzingScreen() {
  ...
  const run = async () => {
    ...
    try {
      ...
      const response = await fetch(target, { method: "POST", headers: { "Content-Type": "image/jpeg" }, body: bytes });
      if (!response.ok) {
        const detail = await response.text().catch(() => "");
        console.warn("Appliance upload error:", response.status, detail);
      ...
      const imageId = await completeUpload({ sessionId, storageId: uploaded.storageId, purpose: mode === "label" ? "label" : "problem", mime: "image/jpeg", size: bytes.byteLength, width: optimized.width, height: optimized.height, checksum: sha256Hex(bytes) });
